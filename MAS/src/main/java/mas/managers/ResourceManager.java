    ...
    private void ExplorationAntLogic(Message m) {
        ...
        if (ant.hasReachedDestination()) {
            ...
            ExplorationAnt new_ant = new ExplorationAnt(
                    ant.getPath(), ant.getRobot_id(), ant.getId(), return_path, ant.getDistance() + 1, ant.getRobotComm());
            ...
            if (this.getPosition().get() == ant.getDestination()) {
                ...
                ExplorationAnt new_ant = new ExplorationAnt(new_path, ant.getRobot_id(), ant.getId(), return_path, roadModel.getDistanceOfPath(new_path).getValue(),
                        ant.getRobotComm());
                ...
                for (CommUser neighbor : neighbors) {
                    ...
                    if (!ant.getPath().contains(neighbor.getPosition().get())) {
                        ExplorationAnt new_ant = new ExplorationAnt(
                                new_path, ant.getRobot_id(), ant.getId(), ant.getRobotComm());
